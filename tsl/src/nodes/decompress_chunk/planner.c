...
{
	fprintf(stderr, "required %s, input tlist:\n", bmsToString(chunk_attrs_needed));
	my_print(scan_tlist);
	/*
	...
	check_for_system_columns(chunk_attrs_needed);
	...
	foreach (lc, scan_tlist)
	{
		...
		char *column_name = get_attname(path->info->compressed_rte->relid,
										compressed_attno,
										/* missing_ok = */ false);

		if (target->resname == NULL)
		{
			target->resname = column_name;
		}
	...
	{
		elog(ERROR,
			 "column '%s' (%d) not found in the compressed scan targetlist",
			 get_attname(path->info->compressed_rte->relid,
							 ...
							 List *decompressed_tlist, List *clauses, List *custom_plans)
{
	...
	build_decompression_map(dcpath, compressed_scan->plan.targetlist,
		dcpath->info->ht_rte->selectedCols);
