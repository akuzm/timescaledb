								  ...
								  Path *subpath, List **sorted_paths, List **hashed_paths)
{
	...
	fprintf(stderr, "adjusted chunk target before grouping is:\n");
	my_print(chunk_target_before_grouping);
	...
	{
		DecompressChunkPath *path_copy = palloc(sizeof(DecompressChunkPath));
		memcpy(path_copy, subpath, sizeof(DecompressChunkPath));
		path_copy->custom_path.path.pathtarget = chunk_target_before_grouping;
		subpath = &path_copy->custom_path.path;
	}
	else
	{
		subpath = (Path *)
...
{
	fprintf(stderr, "the root is:\n");
	my_print(root);
	fprintf(stderr, "input rel is:\n");
	my_print(input_rel);
	fprintf(stderr, "cheapest total path is:\n");
